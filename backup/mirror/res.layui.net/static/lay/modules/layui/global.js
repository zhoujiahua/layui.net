...
layui.define(['code', 'element', 'table', 'util', 'carousel', 'laytpl'], function(exports){
  ...
  ;!function(){
    ...
    form.on('select(component)', function(data){
      ...
      location.href = /^\//.test(value) ? value : ('../../www.layui.net/doc/'+ value);
    });
  }();
    ...
    $('.site-down').on('click',function(e){
      ...
      if(!local.disclaimer){
        ...
        layer.confirm([
          ...
          ,'请先阅读《<a href="/www.layui.net/about/disclaimer.html" target="_blank">layui 开源界面框架免责声明</a>》'
          ,'，再进行下载</div>'
        ].join(''), {
          title: '下载提示'
          ,btn: ['已阅读', '取消']
    ...
    if(elemHome.data('date') === '4-1'){
      ...
      setTimeout(function(){
        layer.photos({
          photos: {
            "data": [{
              "src": "//cdn.layui.net/upload/2018_4/168_1522515820513_397.png",
            }]
          }
          ,anim: 2
          ,shade: 1
